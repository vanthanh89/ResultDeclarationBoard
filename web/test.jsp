...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <script type="text/javascript">
            function getSubjectBySemester() { 
            var e = document.getElementById('semesterId').value;
         
            var url = "${pageContext.request.contextPath}/getSubjectBySemesterId.do?semesterId=" + e;
            ...
            <div class="main_content">
                ...
                <div class="menu">
                    <ul>
                        ...
                        <li><a class="current" href="test.jsp">Manage Re-evaluation<!--[if IE 7]><!--></a></li>
                ...
                <div class="center_content">  
                    ...
                    <div class="right_content">            
                        <center>
                            <h2>Statistic Student by marks in a subject of semester</h2>
                            <html:form action="sp_StudentByMarksInSubject">
                                <jsp:useBean id="dao" class="hibernate.dao.AcademicDepartmentDAO" scope="session"></jsp:useBean>
                                
                                    <table border="0">
                                        <tbody>
                                            <c:if test="${empty requestScope.lstSubject}">
                                            <tr>
                                                <td>Semester :</td>
                                                <td>
                                                    <select  id="semesterId" name="semesterId" style="width: 150px;" onchange="getSubjectBySemester();">
                                                    <c:forEach var="sem" items="${dao.getAllSemester()}"  >
                                                        <option value="${sem.semesterId}">${sem.term}</option>
                                                    </c:forEach>
                                                </select>
                                            </td>
                                        </tr>
                                        <tr>
                                                ...
                                                <html:select property="subjectId" style="width: 150px;" >
                                                    <c:forEach var="sub" items="${dao.getAllSubjectBySemesterId(1)}"  >
                                        ...
                                        <c:if test="${not empty requestScope.lstSubject}">
                                            <tr>
                                                <td>Semester :</td>
                                                <td>
                                                    <select  id="semesterId" name="semesterId" style="width: 150px;" onchange="getSubjectBySemester();">
                                                    <c:forEach var="sem" items="${dao.getAllSemester()}"  >
                                                        <c:if test="${requestScope.semId == sem.semesterId }">
                                                            <option selected="true" value="${sem.semesterId}">${sem.term}</option>
                                                        </c:if>
                                                        <c:if test="${requestScope.semId != sem.semesterId }">
                                                            <option value="${sem.semesterId}">${sem.term}</option>
                                                        </c:if>
                                        ...
                                        <tr>
                                                ...
                                                <html:select property="subjectId" style="width: 150px;" >
                                                    <c:forEach var="sub" items="${requestScope.lstSubject}"  >
                                                        <html:option value="${sub.subjectId}">${sub.subjectName}</html:option>
                                                    </c:forEach>
                                                </html:select>
                                            </td>
                            ...
                            <br />
                            <div>
                                <c:if test="${not empty requestScope.resultSet}">
                                    <table>
                                        <thead>
                                            <th class="header_table">Student ID </th>
                                            <th class="header_table">Student Name </th>
                                        ...
                                        <tbody>
                                            ...
                                            <% while (rs.next()) { %>
                                            <tr>
                                                <td class="vl_table"><%= rs.getInt(1) %></td>
                                                <td class="vl_table"><%= rs.getString(2) %></td>
